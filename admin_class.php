...
$id_empleado = $row['Empleado'];
			$data .= ", user_id ='".$id_empleado."' ";
			$ruta = 'uploads/'.$id_empleado. '/';
			if (!file_exists($ruta)) {
				mkdir($ruta, 0777, true);
			}
			
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$id_empleado."' and name  ='".$name."'")->num_rows;
			if($check > 0){
			...
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$id_empleado."' and name  ='".$name."' and id !=".$id)->num_rows;
			if($check > 0){
				return json_encode(array('status'=>2,'msg'=> 'Folder name already exist'));
			}else{
...
$id_empleado = $row['Empleado'];
		if($_FILES['upload']['tmp_name'] != ''){
					...
					$move = move_uploaded_file($_FILES['upload']['tmp_name'],'uploads/'.$id_empleado.'/'. $fname);
